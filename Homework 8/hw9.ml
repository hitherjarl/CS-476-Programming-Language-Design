...
let terminated (i : tid) (t : thread) : bool = 
  (match t with 
    | (x, y, z, empty_state) -> if (x = i && z = []) then true else false)
...
let step_cmd (funs : fundecls) (threads : thread list)
  ...
  | Fork (x, f) :: rest -> (match funs f with 
          | Some FDecl (_,_,[],c) -> let v = fresh_tid() in 
            (threads @ [(i, rest, k, r); (v, c, [], empty_state)],g)) 
  | Join e :: rest -> (match eval_exp e r g with
          | Some (j) -> (threads @ [(i, rest, k, r)],g)) 
  | _ -> (match step_cmd1 funs lc k r g with
          | Some (lc', k', r', g') -> (threads @ [(i, lc', k', r')], g'))
